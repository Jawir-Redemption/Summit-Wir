...
<section class="max-w-7xl mx-auto px-6 py-10">
  {{-- Tombol kembali --}}
  <a href="{{ route('customer.all-products') }}" class="text-sm text-blue-600 hover:underline flex items-center mb-6">
      ...
      <div class="bg-white shadow rounded-2xl p-6">
        ...
        <div class="space-y-4">
          @foreach($relatedProducts as $related)
            <a href="{{ route('customer.product-detail', $related->id) }}" class="flex items-center gap-4 hover:bg-gray-50 rounded-lg p-2 transition">
              <img src="{{ asset('storage/' . $related->image) }}" alt="{{ $related->name }}" class="w-16 h-16 object-cover rounded">
              <div>
                <p class="font-medium text-sm">{{ $related->name }}</p>
                <p class="text-xs text-gray-500">Rp{{ number_format($related->price, 0, ',', '.') }}</p>
              </div>
            </a>
          @endforeach
        </div>
